...
while True:   
    ...
    E2=[hc_eV/(x*1e-9) for x in Lambda_excite]
    ...
    def fonction_totale(lambdax,lambdai,sigmai,f,Scale) :
        Sum = 0
        ...
        elif type(sigmai) is list:
            for i in range(len(lambdai)) :
                Sum+=gaussienne(lambdax,lambdai[i],sigmai[i],f[i])
        else :
            print("Erreur associé à la valeur de sigma")
        Sum*=Scale
        return Sum
    ...
    G=[]
    for x in Lambda:
        G.append(fonction_totale(x,E,sigma_global,f,1))
